            ...
            if isinstance(subfolder_dict, str):
                if os.path.islink(sub_datapath):
                    ...
                    if os.path.abspath(link_target) == os.path.abspath(subfolder_dict):
                        LOGGER.info(
                            f"There is a symbolic link to {subfolder_dict} at {sub_datapath} already"
                        )
                    else:
                        LOGGER.info(
                            f"Error: {sub_datapath} is a symbolic link to {link_target}, not {subfolder_dict}"
                        )
                        return
                else:
                    if os.path.exists(sub_datapath):
                        LOGGER.info(
                            f"Error: Path {sub_datapath} already exists, cannot create symlink"
                        )
                        return
                    else:
